...
// This file is jmapap's DID-coordination seam, compiled into the default build.
// `go build -tags noanchor` swaps it for anchor_off.go and the relay speaks no
// DID at all — no claim, no release, no /account/did, no /pkarr. The anchor
// package (and whatever it depends on) is imported ONLY here, so that exclusion
// is a compile-time fact: the noanchor binary links none of it.
...
// noanchor build.
func registerAnchorRoutes(mux *http.ServeMux, h *handler, dataDir string) {
